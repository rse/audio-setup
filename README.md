...
For the primary audio processing chain, a few main parameters are used by me.
First, the audio loudness scale knows about three units:

- dBFS (DeciBel relative to Full Scale)
- LUFS (Loudness Unit relative to Full Scale)
- RMS  (Root Mean Square)

The relation is that `dbFS == LUFS ~~ RMS`. LUFS is about the perceived
loudness according to the Fletcher Munson curve while RMS is a plain
mathematical average loundness.

Second, the audio loudness scale can be segmented like this:
...
Third, as a consequence I usually use the following base parameters for the usual audio filters:

- Noise Suppression:
    - Voice Threshold: -42 dBFS
- Frequency Expansion (Soft Gating):
    - Threshold: -54 dBFS
